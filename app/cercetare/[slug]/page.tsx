...
function RenderSection({ section }: { section: ArticleSection }) {
  switch (section.type) {
    ...
    case "heading":
      return (
        <h2 className="text-xl font-bold text-midnight mt-10 mb-4">
          {section.text}
        </h2>
      );
    case "body":
    ...
    case "quote":
      return (
        <blockquote className="my-8 pl-6 border-l-4 border-cobalt/40 bg-cobalt-50 rounded-r-xl py-5 pr-6">
          <p className="text-midnight font-semibold leading-relaxed mb-2">
            &ldquo;{section.text}&rdquo;
          </p>
          {section.author && (
            <cite className="text-xs text-slate-500 not-italic font-medium">{section.author}</cite>
          )}
        </blockquote>
      );
    case "signal-box":
      return (
        <div className="my-7 bg-midnight rounded-xl p-6 border border-white/10">
          <p className="text-[10px] font-bold text-cobalt-light uppercase tracking-widest mb-3">
            {section.label ?? "Date semnal"}
          </p>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-14 pt-8 border-t border-slate-100">
            <p className="text-xs text-slate-400 leading-relaxed">
              Acest material este publicat de ICSCB în scop informativ și de cercetare. Analizele
              se bazează pe date comportamentale din corpusul de monitorizare și nu constituie
              consultanță politică, juridică sau medicală. Opiniile aparțin echipei de cercetare.
            </p>
          </div>
        </div>
      </section>
